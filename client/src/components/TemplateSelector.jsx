    ...
    const templates = [
        {
            id: 'classic',
            name: 'Classic',
            preview: 'A clean, traditional resume layout with clear section and professional typography'
        },
...

        {
            id: 'minimal-image',
            name: 'Minimal Image',
            preview: 'Minimal design with a single image and clean typography for a contemporary look'
        },
                ...
                {templates.map((template) => (
                    <div key={template.id} onClick={() => {onChange(template.id); setIsOpen(false)}} className={` relative p-3 rounded-md cursor-pointer border transition-all ${selectedTemplate === template.id ? 'border-blue-400 bg-blue-100' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-100'}`}>
                        ...
                        <div className='space-y-1'>
                            ...
                            <div className='text-xs text-gray-500 italic mt-2 p-2 bg-blue-50 rounded'>{template.preview}</div>
                        </div>
                    </div >
                ))}
            </div>
