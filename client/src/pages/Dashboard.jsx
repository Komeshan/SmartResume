import { FilePenLineIcon, FolderMinus, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, XIcon } from 'lucide-react'
  ...
  return (
    <div >
        <div className='max-w-7xl mx-auto px-4 py-8'>
          ...
          <p className='text-2xl font-medium mb-6 bg-linear-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome, {user?.name || 'User'}</p>
          ...
          <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
            {allResumes.map((resume, index) => {
              ...
              return (
                <div key={index} onClick={() => navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{background:`linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor: baseColor + '40'}}>
          ...
          {showCreateResume && (
            <div onClick={() => {setShowCreateResume(false); setTitle('')}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
              <form onSubmit={createResume} onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <button type="submit" className='w-full py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors'>Create Resume</button>
          ...
          {showUploadResume && (
            <div onClick={() => {setShowUploadResume(false); setTitle(''); setResume(null)}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
              <form onSubmit={uploadResume} onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-blue-600 ring-blue-600' required/>
                  <div>
                    <label htmlFor="resume-input" className='block text-sm text-slate-700'>Select resume file
                      <div className='flex flex-col items-center justify-center gap-2 p-4 py-10 my-4 border group text-slate-400  border-dashed border-slate-400 rounded-md cursor-pointer hover:border-blue-600 hover:text-blue-700 transition-colors'>
                        {resume ? (
                ...
                <button type="submit" className='w-full py-2 bg-blue-600 text-white flex items-center justify-center gap-2 rounded hover:bg-blue-700 transition-colors' disabled={isLoading}>
          ...
          {editResumeId && (
            <div onClick={() => {setEditResumeId(''); setTitle('')}} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
              <form onSubmit={editTitle} onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-blue-600 ring-blue-600' required/>
                <button type="submit" className='w-full py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors'>Update</button>
